...
if (isset($_POST['signup'])) {
    ...
    $stmt = $conn->prepare("SELECT uname FROM donar WHERE uname=?");
    ...
    if ($count == 0) { // if email is not found add user
        ...
        $sql="INSERT INTO donar(name,mobile,address,uname,password) VALUES(?, ?, ?, ?, ?)";
     
       $stmts = $conn->prepare($sql);
            $stmts->bind_param("sssss", $name, $mobile, $address,$email,$upass);
